
<section class="login_section py-md-1 bgi_2">
<div class="container px-lg-5">
  <div class="row bs_1 py-3">
    <div class="col-md-6 bg_9 login_contentColor rounded-left">
     <div class="login_left_content align-items-start pt-md-5 pt-2 d-flex h-100 justify-content-center">
       <div>
        <div class="text-center"><img src="<?= base_url('assets/lms/'); ?>img/Logo -unboxskills.png" alt="Logo unboxskills" width="100px"></div>
        <h1 class="text-white fw_400 f_32 rt_16 text-center py-2">Login as a <span id="loginTitle">student</span></h1>
        <h2 class="text-white fw_300 f_16 rt_14 text-center">Good to see you again!</h2>
        <h3 class="text-white fw_300 f_16 rt_14 text-center lh_1">By logging into UnboxSkills, you agree to our Terms of use and Privacy Policy.</h3>
       </div>
     </div>
    </div>

    <div class="col-md-6 bg-white px-lg-5 px-2 rounded-right">
      <div class="login1">
         <div onsubmit="userLogin()">

            
      <input type="hidden" id="userType" value="student" >
      <input type="hidden" id="focusPoint" value="login" >

      <div class="changetype d-flex justify-content-center align-items-stretch  pb-2">
        <div class="student py-1 w-md-25 changetypeshow border rounded px-3 text-center cp" onclick="registertype('student')" id="btnstudent">Student</div>
        <div class="teacher pt-1 w-md-25  text-center border rounded px-3 cp" onclick="registertype('teacher')" id="btnteacher">Teacher</div>
      </div>
            <!-- <h2 class="title rt_25 fw_500 text-dark"> Sign In</h2> -->
        <p class="subtitle rt_13 f_14 mt-2 text-secondary">Don't have an account? <a href="<?= base_url(); ?>register"> Sign Up</a></p>

            <!-- <div class="col-12 text-center"><h3>Login Account!</h3></div> -->

            <div class="social-login d-none">
               
               <button class="google-btn" type="button">
                  <a href="" class="text-dark d-flex align-items-center justify-content-between">
                  <img alt="Google" src="data:image/svg+xml;base64," />
                  <p class="btn-text rt_14 f_14 pl-2">Sign in with Google</p>
                  </a>
              </button>

              <button class="fb-btn">
                  <a href="" class="text-dark d-flex align-items-center justify-content-between">
                  <img alt="FB" src="data:image/svg+xml;base64," />
                  </a>
               </button>        

            </div>

            <p class="or text-secondary pb-3"><span class="f_12"> OR </span></p>

            <div class="email-login">
               <label for="email"> <b>Email</b></label>
               <input type="text" placeholder="Enter Email" id="loginEmail" >
               <label for="psw"><b>Password</b></label>
               <input type="password" placeholder="Enter Password" id="loginPass" >
               <input type="hidden" id="loginToken" value="<?php echo $_SESSION['loginToken'] ?>">
            </div>

            <div class="button46 w-100 d-block border-0 mt-2 mb-2 text-center LoginBtn1"  onclick="userLogin()">Log In</div>
            
            <a class="forget-pass rt_14 f_15" href="<?= base_url('/reset-account'); ?>">Forgot password?</a>
            
            <div class="col-md-12  text-center p-3">
                <h5 class="signErrorMsg animate__animated mt-lg-3 mt-1"></h5>
            </div>

         </div>
      </div>
    </div>
  </div>
</div>
</section>

