...
class Ocr_model extends CI_Model
{
    ...
    public function getAttachment($id,$pdf)
    {
      ...
      $output[]= $shellObj->exeCmd('python3 /var/www/html/uat/main.py upload/report/'.$id.'/'.$pdf.' '.$id);
    ...
    {
      $shellObj    = \MTS\Factories::getDevices()->getLocalHost()->getShell('bash', false);
      $output= $shellObj->exeCmd('python3 /var/www/html/uat/main.py upload/report/'.$id.'/'.$name.' '.$id);
      // $output[]=$shellObj->exeCmd('mv ./attachment.pdf ./upload/pdf/'.$id.'/attachment.pdf');
      return $output;
    }
    ...
    {
      $shellObj    = \MTS\Factories::getDevices()->getLocalHost()->getShell('bash', false);
      $cmd='nohup python /var/www/html/uat/new_ocr.py '.$id.' '.$tracking_id.' '.$name.' '.$type.' '.$heads.' '.$Category.' &';
    ...
    {
      $shellObj    = \MTS\Factories::getDevices()->getLocalHost()->getShell('bash', false);
      $cmd='nohup python /var/www/html/uat/mgt_ocr.py '.$id.' '.$tracking_id.' '.$name.'  &';
